...
interface CustomDataTableProps<T> {
  ...
  paginationText?: (pagination: PaginationMeta) => string;
...
const ReturnCartOfAdmin = () => {
  ...
  const fetchSalesPersons = async () => {
    try {
      ...
      const salesPersonsWithReturns: SalesPerson[] = allSalesPersons.filter(
        (sp: SalesPerson) =>
          sp.returnAppliedProducts && sp.returnAppliedProducts.length > 0
      );
  ...
  const handleBulkAction = (action: string) => {
    ...
    console.log(`${action} selected products:`, selectedRecords);
    
    // Extract the _id from each product's productId object
    const productIds = selectedRecords.map(record => record.productId._id);
    
    console.log(`Product IDs for ${action}:`, productIds);
    
    // Here you would call your API to perform the bulk action
    // For example:
    // axios.post(`${backendUrl}/admin/bulk-${action.toLowerCase()}-returns`, { productIds })
  };

  const acceptArrayOfProducts = async (productIds: string[]) => {
        ...
        <div className="panel mt-6">
          <div className="flex justify-between items-center mb-5">
            <h5 className="font-semibold text-lg dark:text-white-light">
              Return Applied Products for {selectedSalesPerson.name}
            </h5>
                  ...
                  render: (record: Product) => (
                    <div className="flex gap-2">
                      <button 
                        ...
                        onClick={() => acceptProduct(record.productId?._id)}
                      >
                        Approve
                      </button>
                      <button 
                        ...
                        onClick={() => rejectingProduct(record.productId?._id)}
                      >
                        Reject
                      </button>
                    </div>
